...
export const getCart = () => async (dispatch, getState) => {
    ...
    try{
         cart = await axios.get('http://localhost:3030/cart/' + id_user)
    }
    catch(err) {
        console.log(err)
        return
...
export const deteleProductInCart = (id_product) => async(dispatch, getState) => {
    if (!getState().userReducers.login.islogin) {
        ...
        try {
            await axios.post('http://localhost:3030/cart/delete', {
                id_user: storeConfig.getUser().id,
...
export const payment = ( address, phone, name,total) => async (dispatch, getState) => {
    ...
    try {
        ...
        console.log(phone);
        console.log(name)
        res = await axios.post('http://localhost:3030/bill/add', {
            id_user: storeConfig.getUser().id,
            address: address,
            phone: phone,
            name: name,
