...
export const getCategory = () => async (dispatch, getState) =>  {
    let res
    try {
        res = await axios.get('http://localhost:3030/category')
    }
    catch (err) {
        return
    }
...
export const getPublisher = () => async (dispatch, getState) => {
    let res
    try {
        res = await axios.get('http://localhost:3030/publisher')
    }
    catch (err) {
        return
    }
...
export const getAuthor = () => async (dispatch, getState) => {
    let res
    try {
        res = await axios.get('http://localhost:3030/author')
    }
    catch(err) {
        return
    }
...
export const getBook = () => async (dispatch, getState) => {
    ...
    let _link = 'http://localhost:3030/book/allbook'
    if(branch === 'category') { 
        _link = 'http://localhost:3030/book/category'
    } else if (branch === 'publisher') {
        _link = 'http://localhost:3030/book/publisher'
    } else if(branch === 'author') {
        _link = 'http://localhost:3030/book/author'
    }
    let res
    try {
